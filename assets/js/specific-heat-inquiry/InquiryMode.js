...
(function (global) {
  ...
  const STEP_CONFIRM_HINTS = {
    ...
    results: "結果が2行以上記録されてから「決定」を押してください（水・鉄など複数回測定）。",
    graph: "測定した物質ごとにグラフを作成してから「決定」を押してください。",
  ...
  class InquiryMode {
    constructor(options) {
      ...
      this.steps = [];
      this.validators = {};
      this.lastRecorded = { material: null, mass: null, heatQ: null };
    ...
    _patch(patch) {
      ...
      if (patch.graphCreatedWater) graphsPatch.water = true;
      if (patch.graphCreatedIron) graphsPatch.iron = true;
      const { graphsCreated, graphCreatedWater, graphCreatedIron, ...rest } = patch;
      ...
      if (Object.keys(graphsPatch).length) {
        ...
        Object.entries(graphsPatch).forEach(([materialKey, created]) => {
          if (created && gStep?.drawMaterialGraph) {
            gStep.drawMaterialGraph(materialKey, this.state.qtCurves, this.state);
          }
        });
      }
    ...
    _syncPlanChecksFromPlanId() {
      ...
      if (!this.state.planChecks || this.state.planChecks.material === false) {
        this.state.planChecks = { ...defaults };
      }
    }

    _isPlanChecked(key, c) {
      if (key === "material") return c.material !== false;
      return !!c[key];
    }
    ...
    updateExperimentControls() {
      this._syncPlanChecksFromPlanId();
      const c = this.state.planChecks || global.InquiryMissions?.DEFAULT_PLAN_CHECKS || { material: true, mass: false, heatQ: false };
      const panels = {
        material: document.getElementById("inqVarMaterial"),
        initialTemp: document.getElementById("inqVarInitialTemp"),
        mass: document.getElementById("inqVarMass"),
        heatQ: document.getElementById("inqVarHeatQ"),
      };
      ...
      const investigate = [];
      if (c.material !== false) investigate.push("物質");
      if (c.mass) investigate.push("質量 m");
      if (c.heatQ) investigate.push("加熱量 Q");
      ...
      const sim = global.SpecificHeatSim;
      if (sim && this.recordingEnabled) {
        const s = sim.getState();
        const now = Date.now();
        const entries = [
          { check: c.material !== false, type: "material", key: "material", val: s.materialKey },
          { check: !!c.mass, type: "mass", key: "mass", val: s.mass },
          { check: !!c.heatQ, type: "heatQ", key: "heatQ", val: s.heatQ },
        ];
    ...
    _ensureMergedResults() {
      if (!InquiryStorage.mergeResultsByInterval) return;
      this.state.results = InquiryStorage.mergeResultsByInterval(this.state.results || []);
    }
    ...
    _refreshUI() {
      ...
      if (expStep?.renderTimeline) expStep.renderTimeline(this.state.timeline);
      if (expStep?.renderHeatLog) expStep.renderHeatLog(this.state.heatMeasureLog);
      ...
      if (resStep?.renderMeasureSessions) {
        resStep.renderMeasureSessions(this.state.heatMeasureLog, []);
      }
    ...
    showReport() {
      if (!this.reportDocEl || !global.InquiryReport) {
        console.error("[探究レポート] reportDocEl または InquiryReport がありません");
      ...
      try {
        ...
        if (!this.reportDocEl.querySelector(".report-render-error")) {
          this.reportDocEl.innerHTML =
            '<div class="report-render-error">' +
    ...
    downloadReportPdf() {
      ...
      global.InquiryReport.downloadPdf(this.reportDocEl, "specific_heat_inquiry_report_" + stamp + ".pdf")
    ...
    measureNow() {
      ...
      if (sim.isAnimating && sim.isAnimating()) {
        this._setMeasureFeedback("加熱中です。完了してから再測定してください。", false);
        return;
      }
      ...
      if (!sim.isAnimating || !sim.isAnimating()) {
        this._setMeasureFeedback("加熱を開始できませんでした。物質・温度・加熱量を確認してください。", false);
      ...
      const finish = () => {
        const s = sim.getState();
        const segmentRows = sim.getLastSessionSegmentResults?.() || [];

        if (segmentRows.length === 0) {
          this._setMeasureFeedback("区間データを取得できませんでした。加熱が完了してから再試行してください。", false);
          this._measuring = false;
          return;
        }

        const measureId = Date.now();
        const pending = segmentRows.map((row, i) => ({ ...row, measureId, segmentIndex: i }));
        const results = InquiryStorage.mergeResultsByInterval([
          ...(this.state.results || []),
          ...pending,
        ]);
        const newKeys = pending.map((row) => [row.materialKey, row.interval].join("|"));

        this.state.results = results;

        if (s.materialKey) {
          const chartPts = sim.getChartData?.() || [];
          if (!this.state.qtCurves) this.state.qtCurves = {};
          this.state.qtCurves[s.materialKey] = chartPts
            .filter((p) => Number.isFinite(p.x) && Number.isFinite(p.y))
            .map((p) => ({ x: p.x, y: p.y }));
        }

        const chartPtsForLog = sim.getChartData?.() || [];
        const sortedPts = chartPtsForLog
          .filter((p) => Number.isFinite(p.x) && Number.isFinite(p.y))
          .slice()
          .sort((a, b) => a.x - b.x);
        const lastPt = sortedPts[sortedPts.length - 1];
        const measureRound = (this.state.heatMeasureLog?.length || 0) + 1;
        const sessionStart = Number(s.sessionStartCelsius ?? segmentRows[0]?.initialCelsius ?? s.celsius);
        const sessionEnd = Number(s.celsius);
        const sessionDeltaT = Math.round(Number(s.deltaT ?? (sessionEnd - sessionStart)) * 10) / 10;
        if (!this.state.heatMeasureLog) this.state.heatMeasureLog = [];
        this.state.heatMeasureLog.push({
          measureId,
          measureRound,
          ...
          heatQ: s.heatQ,
          mass: s.mass,
          sessionStartCelsius: Math.round(sessionStart * 10) / 10,
          sessionEndCelsius: Math.round(sessionEnd * 10) / 10,
          sessionDeltaT,
          cumulativeQ: lastPt ? Math.round(lastPt.x) : 0,
          tempC: Math.round(sessionEnd * 10) / 10,
          at: measureId,
        });
        ...
        if (resStep?.renderMeasureSessions) resStep.renderMeasureSessions(this.state.heatMeasureLog, [measureId]);
        if (resStep?.renderTable) resStep.renderTable(results, newKeys, this.state.planChecks);
        this._refreshUI();

        const intervalList = segmentRows.map((r) => r.interval).join("、");
        const curveNote = (s.materialKey === "water" || s.materialKey === "iron")
          ? " · " + s.materialName + "の加熱曲線を更新"
          : "";
        const msg = "測定完了：" + segmentRows.length + " 区間を記録（" + intervalList + "）" + curveNote;
    ...
    resetExperiment() {
      ...
      this.state.timeline = [];
      this.state.results = [];
      this.state.heatMeasureLog = [];
      this.state.graphsCreated = { water: false, iron: false, aluminum: false, air: false };
      this.state.graphCreatedWater = false;
      this.state.graphCreatedIron = false;
      this.state.qtCurves = {};
      ...
      if (sim) {
        ...
        if (c.material !== false) {
          this.state.timeline.push({ type: "material", from: null, to: s.materialKey, at: now });
          this.lastRecorded.material = s.materialKey;
        }
        if (c.mass) {
          this.state.timeline.push({ type: "mass", from: null, to: s.mass, at: now });
          this.lastRecorded.mass = s.mass;
        }
        if (c.heatQ) {
          this.state.timeline.push({ type: "heatQ", from: null, to: s.heatQ, at: now });
          this.lastRecorded.heatQ = s.heatQ;
        }
      }
    ...
    _setupSimRecording() {
      ...
      if (this.state.timeline.length === 0) {
        const now = Date.now();
        if (this._planEnabled("material")) {
          this.state.timeline.push({ type: "material", from: null, to: s.materialKey, at: now });
          this.lastRecorded.material = s.materialKey;
        }
        if (this._planEnabled("mass")) {
          this.state.timeline.push({ type: "mass", from: null, to: s.mass, at: now });
          this.lastRecorded.mass = s.mass;
        }
        if (this._planEnabled("heatQ")) {
          this.state.timeline.push({ type: "heatQ", from: null, to: s.heatQ, at: now });
          this.lastRecorded.heatQ = s.heatQ;
        }
        InquiryStorage.save(this.state);
        ...
        this.state.timeline.forEach((item) => {
          ...
          if (item.to != null) this.lastRecorded[type] = item.to;
        });
      }
    }
    ...
    _planEnabled(key) {
      const c = this.state.planChecks || {};
      return c[key] !== false && (key === "material" ? c.material !== false : !!c[key]);
    }
    ...
    _onSimUpdate(simState) {
      ...
      if (c.material !== false && simState.materialKey !== this.lastRecorded.material) {
        this._logChange("material", this.lastRecorded.material, simState.materialKey);
        this.lastRecorded.material = simState.materialKey;
      }

      if (c.mass && simState.mass !== this.lastRecorded.mass) {
        this._logChange("mass", this.lastRecorded.mass, simState.mass);
        this.lastRecorded.mass = simState.mass;
      }

      if (c.heatQ && simState.heatQ !== this.lastRecorded.heatQ) {
        this._logChange("heatQ", this.lastRecorded.heatQ, simState.heatQ);
        this.lastRecorded.heatQ = simState.heatQ;
      }
    }

    _resultKey(r) {
      return [r.materialKey ?? "water", r.mass ?? 500, r.heatQ ?? 2000].join("|");
    }
    ...
    resetProgress() {
      ...
      this.welcomeEl?.classList.remove("hidden");
      this.lastRecorded = { material: null, mass: null, heatQ: null };
      if (global.SpecificHeatSim?.reset) global.SpecificHeatSim.reset();
      this._buildSteps();
      this._bindSummaryScreen();
      this._bindReportScreen();
      this._bindExperimentActions();
